...
#define GETPATT(x, mod, patt) x = pPattern.Find(mod, patt);
...
public:
	...
	DWORD m_dwDirect3DDevice = 0x0;
public:
	__inline bool Initialize()
	{
		GETPATT(m_dwDirect3DDevice, "shaderapidx9.dll", "75 22 A1");
		...
		if (const DWORD dwClientMode = pPattern.Find("client.dll", "8B 0D ? ? ? ? 8B 02 D9 05"))
			m_dwClientModeShared = (dwClientMode + 0x02);

		if (const DWORD dwScreenSpaceFx = pPattern.Find("client.dll", "E8 ? ? ? ? 8B 0D ? ? ? ? 8B 01 FF 90 ? ? ? ? 83 F8 01 75 64 8B 0D ? ? ? ? 8B 01 FF 90 ? ? ? ? 8B F0 85 FF 74 0E 8B 17"))
			m_dwPerformScreenSpaceEffects = (dwScreenSpaceFx + 0x01);

		/*if (const DWORD dwGlobalVars = pPattern.Find("engine.dll", "A1 ? ? ? ? 8B 11 68"))
			m_dwGlobalVars = (dwGlobalVars + 0x08);*/

		if (const DWORD dwClientState = pPattern.Find("engine.dll", "68 ? ? ? ? E8 ? ? ? ? 83 C4 08 5F 5E 5B 5D C3"))
			m_dwClientState = (dwClientState + 0x01);

		if (const DWORD dwInitKVal = pPattern.Find("engine.dll", "FF 15 ? ? ? ? 83 C4 08 89 06 8B C6"))
			m_dwInitializeKeyValue = (dwInitKVal - 0x42);

		if (const DWORD dwRandomSeed = pPattern.Find("client.dll", "C7 05 ? ? ? ? ? ? ? ? 5D C3 8B 40 34"))
			m_dwPredictionRandomSeed = (dwRandomSeed + 0x02);

		if (const DWORD dwPredictionPlayer = pPattern.Find("client.dll", "89 3D ? ? ? ? F3"))
